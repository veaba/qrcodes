// QRCode Rust 集成测试工具
// 用于测试 @veaba/qrcode-rust 的功能正确性
//
// 使用方法:
//   cargo run --release --bin test-qrcode-rust

use qrcode_rust::{QRCode, QRCodeOptions, QRErrorCorrectLevel};

fn main() {
    println!("═══════════════════════════════════════");
    println!("  @veaba/qrcode-rust 集成测试套件");
    println!("═══════════════════════════════════════\n");
    ...
    run_test("SVG 颜色设置", || {
        ...
        qr.make_code("Hello");
        let svg = qr.get_svg();
        assert!(svg.contains("#FF0000"), "SVG 应该包含自定义深色");
        assert!(svg.contains("#FFFFFF"), "SVG 应该包含自定义浅色");
    });
    passed += 1;
    ...
    if failed > 0 {
        ...
        println!("✅ @veaba/qrcode-rust 所有测试通过！\n");
    }
}
