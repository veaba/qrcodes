// QRCode Fast 终端输出测试工具
// 专门测试 @veaba/qrcode-fast 的终端输出功能
//
// 使用方法:
//   cargo run --release --bin test-terminal-fast

use qrcode_fast::{QRCode, QRErrorCorrectLevel};

fn main() {
    println!("═══════════════════════════════════════");
    println!("  @veaba/qrcode-fast 终端输出测试");
    println!("═══════════════════════════════════════\n");
    ...
    run_test("L 级别终端输出", || {
        let mut qr = QRCode::with_options(QRErrorCorrectLevel::L);
        qr.make_code("Test");
        let terminal = qr.to_terminal(false, 1);
    ...
    run_test("Q 级别终端输出", || {
        let mut qr = QRCode::with_options(QRErrorCorrectLevel::Q);
        qr.make_code("Test");
        let terminal = qr.to_terminal(false, 1);
    ...
    println!("✅ @veaba/qrcode-fast 终端输出所有测试通过！\n");
}
